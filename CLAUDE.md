...
One cron entry (`*/10`) hits `/api/cron/tick`; `scheduler.due_jobs()` decides
...
| `live_scoring` | 3 min while a match is live | `weekly_stats` |
| `finalize` | once, at the rollover | `weekly_stats` (authoritative) |
| `predict` | once, at the rollover, after finalize | prediction tables |
