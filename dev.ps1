...
#
#   .\dev.ps1                 # start the Flask dev server (default)
#   .\dev.ps1 run             # same as above
#   .\dev.ps1 reset-draft     # wipe rosters/picks/trades -> fresh draft on the current DB
#   .\dev.ps1 seed-mock       # (re)build the 2-league offline mock DB (mock_fantasy.db)
#   .\dev.ps1 logins          # print the local test team logins
#   .\dev.ps1 help
#
...
try:
    ...
    rows = c.execute("SELECT email, team_name FROM users ORDER BY user_id").fetchall()
