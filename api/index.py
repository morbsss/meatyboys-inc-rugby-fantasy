...
from .leagues import (
    ...
    FRONT_ROW_SPOTS, joinable_leagues,
    roster_model, model_draft_picks, model_individual_positions,
    model_starter_count, model_bench_count, squad_quotas,
...
#                      (current_league_id, _roster_model); cursor/db helpers.
#   AUTH & ACCOUNT     /auth, register/login/logout, profile, team name,
#                      league commissioner, password, joinable leagues.
#   SQUAD & LINE-UP    /squad page, /api/state, /api/players (hub),
#                      /api/my-picks, team views, save picks (validate_roster).
#   TRADES             free-agent pickups + user-to-user trades (like-for-like
#                      for positioned/OFDS leagues).
#   DRAFT              snake-draft engine glue: pool, board, pick/auto-pick,
#                      finalize. Per-league via the roster model (api/draft.py).
...
# "Remember me": how long a persistent (permanent) session cookie survives.
...
# name, league commissioner, password change, and the joinable-leagues list.
# ===========================================================================
...
def list_leagues():
    """Joinable leagues for the onboarding chooser (spec §6.1). Currently
    meatyboys only; OFDS is parked until its approach is defined."""
    return jsonify([
        {
        ...
        for cfg in joinable_leagues().values()
    ]), 200
...
def register():
    ...
    # New sign-ups are remembered by default (persistent session cookie).
...
def login():
    ...
    # "Remember me": persist the session cookie for PERMANENT_SESSION_LIFETIME
    # so the user stays signed in across browser restarts. Unchecked → a
    # browser-session cookie that expires when the browser closes.
...
def get_user():
    """Get current logged-in user, including their league for theming and the
    profile-screen fields (commissioner status + whether the team name is still
    editable)."""
    ...
    league_id = ctx['league_id'] if ctx else None
    meta = _get_draft(conn, league_id) if league_id is not None else {}
    commissioner_id = meta.get('commissioner_user_id')
    commissioner_name = _team_name_for_user(conn, commissioner_id) if commissioner_id else None
...
def _team_name_for_user(conn, user_id):
    if not user_id:
        return None
    cursor = _get_cursor(conn)
    cursor.execute('SELECT team_name, username FROM users WHERE user_id = ?', (user_id,))
    row = cursor.fetchone()
    cursor.close()
    if not row:
        return None
    rd = dict(row) if not isinstance(row, dict) else row
    return rd.get('team_name') or rd.get('username')


@app.route('/api/auth/team-name', methods=['POST'])
def update_team_name():
    """Rename the logged-in user's team. Allowed only before the season starts
    (the day of round 1). Cascades the rename to any picks/selections so they
    stay consistent."""
    if not session.get('user_id'):
        return jsonify({'error': 'Not logged in'}), 401

    new_name = (request.get_json(silent=True) or {}).get('team_name', '').strip()
    if not new_name:
        return jsonify({'error': 'Please enter a team name'}), 400
    if len(new_name) > 40:
    ...
    if _team_edit_locked(conn, ctx['league_id']):
        conn.close()
        return jsonify({'error': 'Team names are locked once the season has started'}), 403

    old_name = ctx['team_name']
    ...
    if old_name:
        cursor.execute('UPDATE team_selections SET team_name = ? WHERE team_name = ? AND league_id = ?',
                       (new_name, old_name, ctx['league_id']))
        cursor.execute('UPDATE draft_picks SET team_name = ? WHERE team_name = ? AND league_id = ?',
                       (new_name, old_name, ctx['league_id']))
        cursor.execute('UPDATE team_front_row SET team_name = ? WHERE team_name = ? AND league_id = ?',
                       (new_name, old_name, ctx['league_id']))
        cursor.execute('UPDATE trades SET from_team = ? WHERE from_team = ? AND league_id = ?',
                       (new_name, old_name, ctx['league_id']))
        cursor.execute('UPDATE trades SET to_team = ? WHERE to_team = ? AND league_id = ?',
                       (new_name, old_name, ctx['league_id']))
    conn.commit()
    cursor.close()
    conn.close()
...
def set_commissioner():
    """Claim or resign the league-commissioner role from the profile. Claiming
    is allowed only when the role is vacant; resigning only by the holder. To
    hand over, the current commissioner resigns and another member claims."""
    if not session.get('user_id'):
        return jsonify({'error': 'Not logged in'}), 401

    action = (request.get_json(silent=True) or {}).get('action', 'claim')
    conn = get_db()
    ensure_schema(conn)
    ctx = _user_context(conn)
    if not ctx:
    ...
    current = meta.get('commissioner_user_id')
    cursor = _get_cursor(conn)

    if action == 'resign':
        if current != ctx['user_id']:
            cursor.close(); conn.close()
            return jsonify({'error': 'You are not the commissioner'}), 403
        cursor.execute('UPDATE leagues SET commissioner_user_id = NULL WHERE league_id = ?',
                       (league_id,))
    else:  # claim
        if current and current != ctx['user_id']:
            cursor.close(); conn.close()
            return jsonify({'error': 'This league already has a commissioner'}), 409
        cursor.execute('UPDATE leagues SET commissioner_user_id = ? WHERE league_id = ?',
                       (ctx['user_id'], league_id))
    conn.commit()
    cursor.close()
    conn.close()

    is_comm = (action != 'resign')
    return jsonify({'status': 'success', 'is_commissioner': is_comm}), 200
...
# ===========================================================================
# ====== SQUAD & LINE-UP ======
# The /squad page, /api/state, the player hub (/api/players), the logged-in
# user's picks (/api/my-picks), team views, and saving a line-up. Composition
# rules come from the per-league roster model (validate_roster).
# ===========================================================================

def login_required(view):
    """Gate a page behind a logged-in session. Anonymous visitors are sent to
    the sign-in screen. Server-side, so it holds in every environment (incl.
    local dev) and can't be bypassed client-side."""
    @wraps(view)
...
@login_required
def index():
    # The squad lives at /squad now; / is reserved for a future home page.
...
def api_players():
    ...
    cursor.execute(
        ...
        f'WHERE league_id = ? AND position IN ({pos_ph})',
        (league_id, *positions))
    players = []
    for r in cursor.fetchall():
        d = dict(r) if not isinstance(r, dict) else r
...
def _model_payload(model: dict) -> dict:
    """JSON-friendly roster model for the squad-builder UI."""
    return {
        ...
        'labels': {p: POSITION_LABELS.get(p, p) for p in POSITION_ORDER},
        'order': POSITION_ORDER,
    }


def _team_front_row_view(conn, league_id, team_name, next_round):
    """The team's owned club front-row unit (with captain/bench state) and its
    current matchday front-rowers."""
    cursor = _get_cursor(conn)
    cursor.execute('SELECT club, is_captain, is_bench FROM team_front_row '
    ...
    cursor.close()
    if not row:
        return {'club': None, 'players': [], 'is_captain': False, 'is_bench': False}
    rd = dict(row) if not isinstance(row, dict) else row
    club = rd['club']
    players = _fr_unit_players(conn, league_id, club, next_round)
    return {'club': club, 'players': players,
            'is_captain': bool(rd.get('is_captain')), 'is_bench': bool(rd.get('is_bench'))}
...
def save_picks(team_name):
    ...
    fr_is_captain = 1 if data.get('fr_is_captain') else 0
    fr_is_bench   = 1 if data.get('fr_is_bench') else 0
    ...
    # Validate the individual roster (spec §6.2): exactly ROSTER_SIZE players,
    ...
    ok, msg = validate_roster(selections, _roster_model(conn, league_id))
    if not ok:
        cursor.close()
        conn.close()
    ...
    if fr_club:
        ...
        cursor.execute(
            'INSERT INTO team_front_row (league_id, team_name, round, club, is_captain, is_bench, scraped_at) '
            'VALUES (?, ?, ?, ?, ?, ?, ?)',
            (league_id, user_team, next_round, fr_club, fr_is_captain, fr_is_bench, now))
...
# ===========================================================================
# ====== TRADES ======
# Free-agent pickups (drop one, add a free agent) and user-to-user trades.
# Positioned/OFDS leagues are like-for-like (same position); FR-unit leagues
# (meatyboys) block individual props/hookers (they belong to the club unit).
# ===========================================================================
...
# ===========================================================================
# ====== DRAFT ======
# Snake-draft glue around the pure engine in api/draft.py. The roster model
# sets the pick count + per-position quotas; the FRONT-ROW UNIT helpers below
# (_fr_club_ranks / _available_fr_clubs / _team_fr_club) are meatyboys-only and
# stay inert for OFDS (positioned, individual front-rowers).
# ===========================================================================
...
def _available_players(conn, league_id):
    """Undrafted draftable players for the league's model, ranked by last
    season's total_points. meatyboys excludes props/hookers (the FR unit); OFDS
    includes every position as an individual."""
    drafted = _drafted_player_ids(conn, league_id)
    positions = model_individual_positions(_roster_model(conn, league_id))
    ph = ','.join(['?'] * len(positions))
    cursor = _get_cursor(conn)
    cursor.execute(
        f'SELECT p.player_id, p.name, p.position, p.team AS real_team, '
        f'       COALESCE(ps.total_points, 0) AS rank '
        f'FROM players p LEFT JOIN previous_season ps ON ps.player_id = p.player_id '
        f'WHERE p.league_id = ? AND p.position IN ({ph}) '
...
def _fr_club_ranks(conn, league_id):
    ...
    """
    cursor = _get_cursor(conn)
    cursor.execute("SELECT COUNT(*) FROM players WHERE league_id = ? AND position = 'FR'",
                   (league_id,))
    row = cursor.fetchone()
    has_fr_units = ((row['COUNT(*)'] if isinstance(row, dict) else row[0]) or 0) > 0
    if has_fr_units:
        cursor.execute(
            "SELECT p.team AS club, COALESCE(MAX(ps.total_points), 0) AS rank "
            "FROM players p LEFT JOIN previous_season ps ON ps.player_id = p.player_id "
            "WHERE p.league_id = ? AND p.position = 'FR' GROUP BY p.team",
            (league_id,))
    else:
        cursor.execute(
            "SELECT p.team AS club, COALESCE(SUM(ps.total_points), 0) AS rank "
            "FROM players p LEFT JOIN previous_season ps ON ps.player_id = p.player_id "
...
def _season_start_dt(conn, league_id):
    """Season start = the league's explicit season_start, else the earliest
    scheduled round kickoff. None when no schedule exists yet (pre-season)."""
    meta = _get_draft(conn, league_id)
    candidates = []
    if meta.get('season_start'):
        candidates.append(meta['season_start'])
    cursor = _get_cursor(conn)
    cursor.execute('SELECT MIN(first_kickoff) AS s FROM rounds WHERE league_id = ?', (league_id,))
    row = cursor.fetchone()
    cursor.close()
    earliest = (row['s'] if isinstance(row, dict) else row[0]) if row else None
    if earliest:
        candidates.append(earliest)
    for c in candidates:
        try:
            return _as_utc(datetime.fromisoformat(str(c).replace('Z', '+00:00')))
...
def _record_pick_and_advance(conn, league_id, state, team, player_id, fr_club, is_auto):
    ...
    next_pick = pick_no + 1
    if next_pick > draft_engine.total_picks(order, model):
        cursor.execute(
...
def _finalize_draft(conn, league_id, order):
    """Materialise drafted squads for the first round: up to 15 individuals into
    team_selections + (if drafted) the optional club front-row unit into
    team_front_row. A team that skipped the FR unit drafts an extra individual,
    so cap the fielded squad to ROSTER_SIZE (10 starters + 5 bench); any surplus
    individual stays owned as depth (subbable later)."""
    next_round = get_next_round(conn, league_id)
    model = _roster_model(conn, league_id)
    now = datetime.now(timezone.utc).isoformat()
    cursor = _get_cursor(conn)
...
def api_draft():
    ...
    on_clock = (draft_engine.team_on_clock(order, state['current_pick'], model)
    ...
    fr_unit = bool(model.get('fr_unit'))
    your_fr = _team_fr_club(conn, league_id, your_team) if (your_team and fr_unit) else None
    your_needs = draft_engine.unmet_needs([p['position'] for p in your_roster], model)
    ...
    resp = {
        ...
        'total_picks': draft_engine.total_picks(order, model) if order else 0,
        'on_clock': on_clock,
        ...
        'your_fr': your_fr,
        'fr_unit': fr_unit,
        'need_fr': fr_unit and bool(your_team) and your_fr is None,
        ...
        'available_fr': _available_fr_clubs(conn, league_id) if fr_unit else [],
        'roster_size': model_starter_count(model) + model_bench_count(model),
        'bench_count': model_bench_count(model),
        'draft_picks': model_draft_picks(model),
    }
    conn.close()
    return jsonify(resp)
...
def _make_pick(conn, league_id, player_id, fr_club, by_user_team, is_auto):
    ...
    order = state['draft_order']
    team = draft_engine.team_on_clock(order, state['current_pick'], model)
    if team is None:
    ...
    fr_clubs = _available_fr_clubs(conn, league_id) if fr_unit else []
    has_fr = _team_fr_club(conn, league_id, team) is not None if fr_unit else False
    owned = [p['position'] for p in _team_owned(conn, league_id, team)]
    label = None

    if is_auto or (player_id is None and not fr_club):
        pick = draft_engine.auto_pick(available, owned, model)
        if not pick:
...
def api_draft_autopick():
    ...
    model = _roster_model(conn, league_id)
    state = _get_draft(conn, league_id)
    on_clock = draft_engine.team_on_clock(state['draft_order'], state['current_pick'], model)
...
# ===========================================================================
# ====== COMPETITION ======
# League pages (/competition, /fixtures, /finals) and the /api/competition
# table: dynamic fixtures, weekly scores (get_team_score), standings, playoffs.
# ===========================================================================
...
# ===========================================================================
# ====== SCHEDULER & CRON ======
# Timezone-aware ingestion jobs (rounds, real lineups, live + final scoring),
# driven by /api/cron/tick. Logic lives in api/scheduler.py + api/ingest.py.
# ===========================================================================
